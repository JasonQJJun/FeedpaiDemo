/**
 * Created by ljunb on 16/5/25.
 *
 * 切记  注释不能写在JS里面  否则会报错
 */
import GlobalContants from './common/globalContants'
import React from 'react'; // 导入 react
import {NetInfo, Animated, StyleSheet, View, Text, Platform} from 'react-native'; // 导入 View组件
import {Provider} from 'react-redux';
import store from './store/store';
import Constants from './common/constants';
import NetInfoDecorator from './common/NetInfoDecorator'
import App from './containers/app'; // 导入 app   生成app 组件
global.Common = Constants;

@NetInfoDecorator
export default class Root extends React.Component {
    //状态  一般来说，你需要在constructor中初始化state
    constructor(props) {
        super(props);
        this.state = {
            promptPosition: new Animated.Value(0)
        }
    }
    // 判断网络
    componentWillReceiveProps(nextProps) {
        const {isConnected} = nextProps
        // 无网络
        if (!isConnected) {
            // 动画
            Animated.timing(this.state.promptPosition, {
                toValue: 1,
                duration: 200
            }).start(() => {
                setTimeout(() => {
                    Animated.timing(this.state.promptPosition, {
                        toValue: 0,
                        duration: 200
                    }).start()
                }, 2000);
            })
        }
    }

    render() {
        let positionY = this.state.promptPosition.interpolate({
            inputRange: [0, 1],
            outputRange: [-30, Platform.OS === 'ios' ? 20 : 0]
        });
        // 显示APP 组件
        return (
            <View style={{flex: 1}}>
                <Provider store={store}>
                    <App />
                </Provider>
                <Animated.View style={[styles.netInfoView, {top: positionY}]}>
                    <Text style={styles.netInfoPrompt}>网络异常，请检查网络稍后重试~</Text>
                </Animated.View>
            </View>
        )
    }
}

const styles = StyleSheet.create({
    netInfoView: {
        justifyContent: 'center',
        alignItems: 'center',
        height: 30,
        position: 'absolute',
        right: 0,
        left: 0,
        backgroundColor: Constants.colors.themeColor
    },
    netInfoPrompt: {
        color: 'white',
        fontWeight: 'bold'
    }
});